...
public class CMServerApp {
	...
	public static void main(String[] args) {
		...
		//애플리케이션 cm서비스 액세스하려면 stub모듈을 통해 인터렉션함
		//stub안에 cm이 제공하는 서비스의 모듈들 제공
		//cm의 api쓰기위해 스텁먼저 선언
		
		//서버 애플리케이션 - 서버 스텁, 클라이언트 애플리케이션 - 클라이언트 스텁
		//주요 통신 기능 클라이언트에
		// 애플리케이션이 cm 서버 stub객체를 얻음
		CMServerStub cmStub = server.getServerStub();
		////애플리케이션이 이 이벤트받아 처리하고 싶으면 
		//애플리케이션 레벨의 cm 이벤트 핸들러를 등록
		//서버와 애프리케이션이 자신의 이벤트 핸들러 등록하면 cm이 먼저 이벤트 처리하고
		//이 이벤트를 애플리케이션의 이벤트 핸들러에 보냄
		//상위 레이어 -> 하위레이어 서비스 이용.
		//거꾸로 하위 레이어 cm에서 애플리케이션이 등록한 이벤트 핸들러의 메서드 호출
		//하위 레이어가 상위 레이어의 메서드 호출(콜백함수) \.. 자바 gui의 이벤트 핸들러와 비슷.
		// 꼭 필요 파라미터로 애플리케이션의 이벤트핸들러  객체 넘김
		cmStub.setAppEventHandler(server.getServerEventHandler());
		//서버 cm을  스타트
		//cm 자체적으로 통신과 관련된 여러 기능들 초기화
		//cm의 서버는 다른 cm노드가 보내는 메세지 다 수신하고 처리함
		//cm 내부에서 처리하고 애플리케이션으로 전달
		server.startCM();
		
		//이벤트 받는 것은 어떻게 하는가? - async로 구현되어있음
